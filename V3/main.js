...
ipcMain.on('create-tab', (event, data) => {
  ...
  // Listen for console messages to intercept element selection
  view.webContents.on('console-message', (event, level, message, line, sourceId) => {
    if (message.startsWith('__AI_ELEMENT_SELECTED__')) {
      try {
        const jsonData = message.replace('__AI_ELEMENT_SELECTED__ ', '');
        const data = JSON.parse(jsonData);
        console.log('🎯 Element selected in BrowserView (via console):', data.selector);
        ...
        console.error('Failed to parse element selection data:', err);
      }
    }
  });
...
// RE-ENABLED FOR V2 - Used by AI Router
...
// Forward element selection from BrowserView to main window
ipcMain.on('element-selected', (event, data) => {
  console.log('🎯 Element selected in BrowserView:', data.selector);
  // Forward to main window (renderer process with AI sidebar)
  if (mainWindow && mainWindow.webContents) {
    mainWindow.webContents.send('element-selected', data);
  }
});
...
// RE-ENABLED FOR V2 - Used by AI Router for page modifications
